...
-- Modifications made by Morn to add support for his event system.
-- ----------------------------------------------------------
...
-- ----------------------------------------------------------
-- wait.event: we call this to wait for an event to be triggered
-- ----------------------------------------------------------
function event (target, etype)
  -- Following the same pattern above
  local id = "wait_event_" .. etype .. "_" .. GetUniqueNumber ()
  threads [id] = assert (coroutine.running (), "Must be in coroutine")

  -- This is a local function here because it needs access to the
  -- event target.
  local function event_resume (event)
    require('tprint')(threads)
    local thread = threads [id]
    threads [id] = nil
    coroutine.resume (thread, event)
    -- one-shot! de-register the handler.
    target:deregister (etype, event_resume)
  end
  -- Register an event handler
  target:on (etype, event_resume)

  -- Stop and wait for the event
  return coroutine.yield()
end
